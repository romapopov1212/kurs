...
void MapVisualizerS::display() const {
    textEdit_->append("----------------------------------------------------------");
...
void TreeVisualizerS::printHelper(Leaf* node, QString indent, bool last) {
    if (node != tree_->getNullLeave()) {
        ...
        // Visualize the Map associated with this Leave node
